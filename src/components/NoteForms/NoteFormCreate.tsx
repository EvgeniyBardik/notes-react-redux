...
const NoteFormCreate: React.FC = () => {
  ...
  const changeHandlerCreate = (e: React.ChangeEvent<HTMLFormElement>) => {
    ...
    if (newNote.name === "") {
      ...
      const newNoteForSave:noteIn = {...newNote, created: getDateCreated(), id: uniqid()}
  ...
  return (
    <>
      <div className="w-full max-w-md mt-6 mb-5 mx-auto">
        ...
        >
          <h1 className="font-semibold text-2xl text-center mt-2 mb-8">Create Note</h1>
          <div className="mb-4">
            <label
            ...
            />
          <p className="text-red-500 absolute botton-1 left-1 text-xs italic">{error}</p>
          </div>
          </div>
    ...
    <textarea onChange={changeHandlerContent}
      className="
        h-48
        form-control
        block
    ...
    ></textarea>
      ...
      <input className="form-check-input appearance-none h-4 w-4 border border-gray-300 rounded-xl bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer" type="checkbox" value="" id="active" defaultChecked onChange={changeHandlerActive} />
      <label className="form-check-label text-gray-700 text-sm font-bold mb-2 inline-block " htmlFor="flexCheckDefault">
        Active
      </label>
    </div>
