    ...
    const active = useTypedSelector((state) => state.app.active);
        return (
            <div className="grid grid-cols-12 py-1 items-center bg-emerald-200 text-sm text-gray-500 md:text-base lg:text-lg rounded-lg shadow-md my-4">
                ...
                <div className={["col-span-3 hover:cursor-pointer hover:font-bold transition-all duration-500", active?"underline":null].join(' ')} onClick={() => dispatch(ShowActive())}>Active</div>
                <div className={["col-span-3 hover:cursor-pointer hover:font-bold transition-all duration-500", !active?"underline":null].join(' ')} onClick={() => dispatch(ShowArchived())}>Archived</div>
            </div>
        )
}
