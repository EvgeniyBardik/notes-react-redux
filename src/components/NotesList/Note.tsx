...
const Note: React.FC<NoteItemProps> = ({ note }) => {
  ...
  return (
    ...
    >
      ...
      <div className="md:col-span-1 text-sm lg:text-lg xl:col-span-2">
        {dates(note.content)}
      </div>
      <div className=" col-start-10 col-end-13 md:col-span-2 xl:col-span-1 grid grid-cols-3 md:items-center md:justify-items-center">
        <Link to={`/edit/${note.id}`} className="justify-self-center">
          <i className=" material-icons text-2xl md:text-2xl lg:text-3xl hover:text-amber-500 duration-500 transform transition hover:-translate-y-0.5">
            create
          </i>
        </Link>
        <button className="">
